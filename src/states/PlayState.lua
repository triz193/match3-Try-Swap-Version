...
function PlayState:trySwapTiles(tile1, tile2)
    ...
    local oldBoard = self.board:clone()
    ...
    -- ensure that the gridX and gridY properties of the tiles in oldBoard match those of the tiles in the original board
    oldBoard.tiles[tile1.gridY][tile1.gridX] = oldBoard.tiles[tile2.gridY][tile2.gridX]
    oldBoard.tiles[tile2.gridY][tile2.gridX] = oldBoard.tiles[tempY][tempX]

    if not matches then
        -- If no matches, revert the board to its old state
        self.board = oldBoard
        gSounds['error']:play()
    else
        -- If there are matches, proceed with the game
        self.canInput = false
        self:calculateMatches()
    end
end
